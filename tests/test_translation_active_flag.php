...
assert(substr_count($moduleSource, '$this->GetEffectiveSelectedLanguage(') === 7, 'the effective-language helper must be used at all 7 write sites (ApplyLanguage x2 for names+texts, ApplyAutomationsLanguage, ApplyGreetingLanguage, ApplyChartsLanguage, ApplyEnumerationOptionsToVariable, and since build 164 ApplyForkedProfileToVariable - the second fork path for variables without a variable action)');
