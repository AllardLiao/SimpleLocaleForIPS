...
assert(substr_count($moduleSource, '$this->GetEffectiveSelectedLanguage(') === 9, 'the effective-language helper must be used at all 7 write sites (ApplyLanguage x2 for names+texts, ApplyAutomationsLanguage, ApplyGreetingLanguage, ApplyChartsLanguage, ApplyEnumerationOptionsToVariable, ApplyForkedProfileToVariable) AND in ComputeActiveLanguageContentFingerprint - since build 166, because a fingerprint that ignores the flag decides wrongly whether ApplyLanguage needs to run - AND in StageRenamedObjectNames since build 212, which must know whether a row showed its original text');
